  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      <button onClick={e => { e.stopPropagation(); onChange(Math.max(min, value - 1)); }}
        style={{ width: 24, height: 24, borderRadius: '50%', border: `1.5px solid oklch(80% 0.04 50)`, background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: '"Nunito",sans-serif', fontSize: 14, fontWeight: 700, color: P.muted }}>−</button>
...
function CoinBurst({ coins, onDone }) {
  React.useEffect(() => { const t = setTimeout(onDone, 1100); return () => clearTimeout(t); }, []);
  return (
    <div style={{ position: 'absolute', right: 44, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', display: 'flex', alignItems: 'center', gap: 3, animation: 'coinEarn 1.05s ease-out forwards', zIndex: 20 }}>
      <span style={{ fontSize: 14 }}>🪙</span>
      <span style={{ fontFamily: '"Nunito",sans-serif', fontWeight: 900, fontSize: 14, color: P.terra }}>+{coins}</span>
    </div>
  );
}
...
function TaskRow({ task, onComplete, onEdit, onRemove }) {
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: task.done ? 'oklch(94% 0.01 55)' : P.white, borderRadius: 18, marginBottom: 10, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', opacity: task.done ? 0.6 : 1, transition: 'all 0.15s' }}>
      {showBurst && <CoinBurst coins={task.coins} onDone={() => setShowBurst(false)} />}
...
function SectionLabel({ children }) {
  return <div style={{ fontFamily: '"Nunito",sans-serif', fontSize: 11, fontWeight: 700, color: P.muted, letterSpacing: 2.5, textTransform: 'uppercase', padding: '14px 4px 10px' }}>{children}</div>;
}

export function TasksScreen({ tasks, coins, completedTasks = [], onComplete, onAdd, onEdit, onRemove, onCoinsEdit }) {
    ...
    <div style={{ height: '100%', overflowY: 'auto', paddingBottom: 24 }}>
      ...
      <div style={{ padding: '20px 22px 18px', background: P.white, marginBottom: 2 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 4 }}>
          ...
          {editCoins ? (
            <input value={coinDraft} onChange={e => setCoinDraft(e.target.value.replace(/\D/g, ''))}
      ...
      <div style={{ padding: '0 16px' }}>
          ...
          <div style={{ background:P.white, borderRadius:18, padding:'16px', marginBottom:10, boxShadow:'0 2px 10px rgba(0,0,0,0.08)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              ...
              <div style={{ display:'flex', gap:8 }}>
                ...
                <button onClick={() => { if(!newText.trim())return; onAdd(newText.trim(),newCoins); setNewText(''); setShowAdd(false); }} style={{ background:P.terra, color:'white', border:'none', borderRadius:12, padding:'8px 16px', fontFamily:'"Nunito",sans-serif', fontSize:13, fontWeight:700, cursor:'pointer' }}>Add</button>
              </div>
            </div>
          </div>
        )}

        <SectionLabel>Quick Earn</SectionLabel>
        {!showOneOff ? (
          ...
          <div style={{ background:P.white, borderRadius:18, padding:'16px', marginBottom:10, boxShadow:'0 2px 10px rgba(0,0,0,0.08)', border:'1.5px solid oklch(90% 0.06 66)' }}>
            <input value={ofText} onChange={e => setOfText(e.target.value)} placeholder="What did you do?" autoFocus
              style={{ width:'100%', fontFamily:'"Nunito",sans-serif', fontSize:15, border:'none', outline:'none', marginBottom:12, color:P.ink, background:'transparent' }} />
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                <span style={{ fontSize:11 }}>🪙</span>
        ...
        {completed.length > 0 && (
          <>
            ...
            {completed.map(t => <TaskRow key={t.id} task={t} onComplete={onComplete} onEdit={onEdit} onRemove={onRemove} />)}
          </>
        )}
        ...
        {completedTasks.length > 0 && (
          <div style={{ marginTop: 8 }}>
            <div onClick={() => setShowHistory(h => !h)}
              style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 4px 10px', cursor:'pointer' }}>
              <div style={{ fontFamily:'"Nunito",sans-serif', fontSize:11, fontWeight:700, color:P.muted, letterSpacing:2.5, textTransform:'uppercase' }}>History</div>
              <span style={{ fontSize:13, color:P.muted, transition:'transform 0.2s', display:'inline-block', transform: showHistory ? 'rotate(180deg)' : 'none' }}>▾</span>
            </div>
            {showHistory && (
              <div>
                {completedTasks.map((t, i) => (
                  <div key={t.id ?? i} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 14px', background:P.white, borderRadius:14, marginBottom:6, opacity:0.75 }}>
                    <span style={{ fontSize:13 }}>✓</span>
                    <span style={{ fontFamily:'"Nunito",sans-serif', fontSize:13, fontWeight:600, color:P.ink, flex:1 }}>{t.text}</span>
                    <span style={{ fontFamily:'"Nunito",sans-serif', fontSize:11, color:'oklch(54% 0.08 55)', fontWeight:700 }}>+{t.coins}🪙</span>
                    <span style={{ fontFamily:'"Nunito",sans-serif', fontSize:10, color:P.muted }}>{new Date(t.ts).toLocaleDateString(undefined,{month:'short',day:'numeric'})}</span>
                  </div>
                ))}
              </div>
            )}
